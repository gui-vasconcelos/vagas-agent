def generate_report(jobs):
    by_cat = {"Forte": [], "Possível": [], "Anotar": [], "Fora": [], "Erro": []}
    for j in jobs:
    ...
    lines.append(
        f"**Resumo:** {len(by_cat['Forte'])} fit forte · "
        f"{len(by_cat['Possível'])} possível · "
        f"{len(by_cat['Anotar'])} anotar · "
        f"total analisado: {len(jobs)}"
    )
    lines.append("")

    for cat, emoji in [("Forte", "🟢"), ("Possível", "🟡"), ("Anotar", "⚪")]:
        items = by_cat[cat]
        if not items:
            continue
    ...
    if not any(by_cat[c] for c in ("Forte", "Possível", "Anotar")):
